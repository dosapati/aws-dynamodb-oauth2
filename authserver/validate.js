...
validate.userExistsCallback = (user,callback) => {
	  if (user == null) {
	    validate.logAndThrow('User does not exist');
	  }
...
validate.clientExistsCallback = (client,callback) => {
	  if (client == null) {
	    validate.logAndThrow('Client does not exist');
	  }
...
validate.tokenCallback = (token, accessToken,callback) => {
	try{
	  utils.verifyToken(accessToken);
	}catch(err){
	...
	async.waterfall([
		  function(callback){
			  db.users.findCallbackById(token.userID,callback) 
		...
		// token is a client token
		  ...
		  async.waterfall([
			  function(callback){
				  db.clients.findCallbackById(token.clientID,callback) 
			  },function(client,callback){
				  validate.clientExistsCallback(token.clientID,callback) 
			  },
		  ],function(err,result){
			  if(err){
				  callback(null,null);
...
validate.clientExistsForHttpCallback = (token,client,callback) => {
	  if (client == null) {
	    const error  = new Error('invalid_token');
	    error.status = 400;
	    throw error;
